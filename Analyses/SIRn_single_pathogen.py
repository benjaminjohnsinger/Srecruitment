...
CONTACT = np.genfromtxt('Data/Processed/contact_matrices/KP_contact_all_US_Census.csv', delimiter=',')
def BETA_PC(t):
    return (1.415*23.25*4.3/12.99)*CONTACT
...
def deltas(t,state,params):
    ...
    delta[0:NAG] = (1-VAX(t,S_VAX))*BIRTH_RATE*pop_size*np.concatenate((np.ones(1),np.zeros(NAG-1))) - WANE_UP[0]*state[0:NAG]\
    ...
    for i in range(N_S):
        delta[(3*i+1)*NAG:(3*i+2)*NAG] = VAX(t,i)*BIRTH_RATE*pop_size*np.concatenate((np.ones(1),np.zeros(NAG-1)))\
