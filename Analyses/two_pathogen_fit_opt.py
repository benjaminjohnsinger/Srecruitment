...
## Initial conditions
STATE0 = jnp.zeros(2*(2*N_S+2)*NAG)
...
def likelihood(x):
    ...
    sim_params1["WANE"] = jnp.array([0.0,x[0],0.0])
    ...
    if "Influenza" in pathogen1:
        ...
        sim_params1["S_REL"] = jnp.array([1,x[n],x[n]*x[n+1]])
        pobsrel1 = jnp.array([1,0.46,0.31]) # Henderson 1979
        n += 2
    else:
        sim_params1["S_REL"] = jnp.array([1,x[n],x[n]*x[n+1]])
        pobsrel1 = jnp.array([1,x[n+2],x[n+2]*x[n+3]])
        n += 4
    ...
    obs_age1 = jnp.array([x[n],x[n+1],x[n+2],x[n+3],x[n+4],x[n+5],x[n+6]])
    n += 7

    sim_params2["WANE"] = jnp.array([0.0,x[0],0.0])
    ...
    if "Influenza" in pathogen2:
        ...
        sim_params2["S_REL"] = jnp.array([1,x[n],x[n]*x[n+1]])
        pobsrel2 = jnp.array([1,0.46,0.31])
        n += 2
    else:
        sim_params2["S_REL"] = jnp.array([1,x[n],x[n]*x[n+1]])
        pobsrel2 = jnp.array([1,x[n+2],x[n+2]*x[n+3]])
        n += 4
    sim_params2["P_OBS"] = pobsrel2
    obs_age2 = jnp.array([x[n],x[n+1],x[n+2],x[n+3],x[n+4],x[n+5],x[n+6]])
    n += 7
    interference1to2 = x[n]
    interference2to1 = x[n+1]

    try:
        arguments = [NAG, N_S, sim_params1["AGING_RATE"], sim_params1["BIRTH_RATE"], sim_params1["arrivals"], sim_params1["contact"], jnp.array([interference2to1,interference1to2]), \
...
ln = len(x2)
x2 = jnp.delete(x2, jnp.s_[(ln-14):(ln-7)])
x0 = jnp.concatenate((x1,x2,jnp.ones(2)))
