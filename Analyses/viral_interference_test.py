...
def categorize_pathogens(pathogen):
    ...
    if "PARAINFLUENZA" in pathogen:
    ...
    elif ("INFLUENZA A" in pathogen) or ("INFLUENZA VIRUS A" in pathogen):
        return "InfluenzaA"
    elif ("INFLUENZA B" in pathogen) or ("INFLUENZA VIRUS B" in pathogen):
        return "InfluenzaB"
        ...
        return "Pertussis"
    elif "MYCOPLASMA PNEUMONIAE" in pathogen:
        return "M.pneumoniae"
    elif "CHLAMYDOPHILA PNEUMONIAE" in pathogen:
        return "C.pneumoniae"
    else:
        return "Other"
    ...
    for i, pathogen1 in enumerate(pathogens_of_interest):
        for pathogen2 in pathogens_of_interest[i+1:]:
            ...
            if load:
                ...
                pt.to_csv('Data/Processed/positivity_table_'+pathogen1+pathogen2+period+["","_restrictive"][restrictive]+'_extended.csv', index=False)
            ...
            results_df.to_csv('Data/Processed/viral_interference_CMH_tests2' + ["", "_prevalence_correction"][proportion_symptomatic != []] + ["", "_restrictive"][restrictive] + ["", "_Mbias"][bias] + "_threshold" + str(threshold) + '_extended.csv', index=False)
...
def M_bias(positivity_table, pathogenA, pathogenB, hospA=0.05, p_hospA=0.85, hospB=0.05, p_hospB=0.85):
...
pathogens_of_interest = ["M.pneumoniae", "C.pneumoniae", "Pertussis", "InfluenzaA", "InfluenzaB","SARS-CoV-2", "Enterovirus", "RSV", "Metapneumovirus", "Adenovirus", "Parainfluenza"]
# pathogens_of_interest = ["InfluenzaA","InfluenzaB","SARS-CoV-2", "Rhinovirus", "RSVA", "RSVB", "HMPV", "Adenovirus", "Influenza", "Respirovirus", "Rublavirus"]
...
"Rublavirus": 0.85,
"HMPV": 0.85,
"Metapneumovirus": 0.85,
"Adenovirus": 0.85,
"Enterovirus": 0.85,
"SARS-CoV-2": 0.41} # Proportion recieving tests in data who are hospitalized
...
if __name__ == "__main__":
    ...
    # # test_counts = pd.read_csv('Data/Processed/KPSC_test_counts_extended.csv', index_col=0)
    # # bar chart
    # br = test_counts.plot(kind='bar', figsize=(10, 5), color="silver")
    # # log y
    # plt.yscale('log')
    # # 45 degree x labels
    # plt.xticks(rotation=45, ha='right')
    # # labelled dotted lines a y = 260893, 206084, and 54845
    # plt.plot([3, 14], [260893, 260893], color='#648FFF', linestyle='--', label='260893')
    # plt.plot([15, 17], [206084, 206084], color='#DC267F', linestyle='--', label='206084')
    # plt.plot([19, 23], [54845, 54845], color='#FFB000', linestyle='--', label='54845')
    # # add text to lines
    # plt.text(3, 260893, '260893', color='#648FFF', ha='left', va='bottom')
    # plt.text(15, 206084, '206084', color='#DC267F', ha='left', va='bottom')
    ...
    # plt.savefig('Figures/KPSC_test_counts_extended.png', dpi=300)
    ...
    # clinical_data = pd.concat([old_clinical_data, new_clinical_data], ignore_index=True)
