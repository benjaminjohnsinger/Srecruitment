...
NAG = len(PITZER_AGE_GROUPS)

# Create matrix of population overlap between age groups
AGE_MAP = np.zeros((NAG,16))
for age in range(1200):
    AGE_MAP[np.where([age in group for group in PITZER_AGE_GROUPS])[0][0],np.where([age in group for group in PREM_AGE_GROUPS])[0][0]] += AGE_POP_norm[age]
# Columns sum to 1
...
PITZER_HOME = np.dot(np.dot(AGE_INC,PREM_HOME),AGE_MAP.T)
PITZER_WORK = np.dot(np.dot(AGE_INC,PREM_WORK),AGE_MAP.T)
PITZER_SCHOOL = np.dot(np.dot(AGE_INC,PREM_SCHOOL),AGE_MAP.T)
PITZER_OTHERS = np.dot(np.dot(AGE_INC,PREM_OTHERS),AGE_MAP.T)
PITZER_ALL = np.dot(np.dot(AGE_INC,PREM_ALL),AGE_MAP.T)
...
np.savetxt('Data/Processed/contact_matrices/Pitzer_contact_home_US_Census.csv',PITZER_HOME,delimiter=',')
np.savetxt('Data/Processed/contact_matrices/Pitzer_contact_work_US_Census.csv',PITZER_WORK,delimiter=',')
np.savetxt('Data/Processed/contact_matrices/Pitzer_contact_school_US_Census.csv',PITZER_SCHOOL,delimiter=',')
np.savetxt('Data/Processed/contact_matrices/Pitzer_contact_others_US_Census.csv',PITZER_OTHERS,delimiter=',')
np.savetxt('Data/Processed/contact_matrices/Pitzer_contact_all_US_Census.csv',PITZER_ALL,delimiter=',')