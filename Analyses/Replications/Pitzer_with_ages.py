...
def dStdt(St,t):
    dSt = np.zeros(len(St))
    foi = (1+b1*np.cos(2*np.pi*(t/12-phi)))*beta*c*np.sum((St[2*al:3*al]+p2*St[5*al:6*al]+pA*St[8*al:9*al]))
    dSt[0:al] = (1-v[int(t),:])*np.log(1+B[int(t)-1,:])*np.sum(St)/12 - w0*St[0:al]  - u*St[0:al] + np.concatenate((np.zeros(1), u[:-1]*St[0:al-1]))
    ...
    dSt[4*al:5*al] = v[int(t),:]*np.log(1+B[int(t)-1,:])*np.sum(St)/12 - s1*foi*St[4*al:5*al] + w1*St[3*al:4*al] - u*St[4*al:5*al] + np.concatenate((np.zeros(1), u[:-1]*St[4*al:5*al-1]))
    ...
    dSt[7*al:8*al] = -s2*foi*St[7*al:8*al] + w1*St[6*al:7*al] + w2*St[9*al:10*al] - u*St[7*al:8*al] + np.concatenate((np.zeros(1), u[:-1]*St[7*al:8*al-1]))
...
foi = beta*(1+seasonality*np.cos(2*np.pi*(np.arange(0,T)/12-phi)))*np.sum(c*(result[:,2*al:3*al]+p2*result[:,5*al:6*al]+pA*result[:,8*al:9*al]), axis=1)
H = h*d1*foi*np.sum(result[:,al:2*al], axis=1)+h*d2*s1*foi*np.sum(result[:,4*al:5*al], axis=1)
...
# for i in range(al):
#     plt.plot(np.sum([result[:,j*al+i] for j in range(10)],axis=0), label=str(i),color=viridis(i/al))
# # log y axis
# # plt.xscale('log')    
# plt.yscale('log')
