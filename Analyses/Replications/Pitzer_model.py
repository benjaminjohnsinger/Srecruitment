...
# Initial conditions
St0 = jnp.array([2.9e7/960, 2.9e7*959/960, 1, 0, 0, 0, 0, 0, 0, 0])
...
def dStdt(St,t):
    ...
    N = sum(St)
    beta = beta0*(1+seasonality*jnp.cos(2*jnp.pi*(t/12-0.636)))
    ...
    return(jnp.array([dM, dS0, dI1, dR1, dS1, dI2, dR2, dS2, dIA, dRA]))

result = sp.integrate.odeint(dStdt, St0, jnp.arange(0,T))

foi = beta0*(1+seasonality*jnp.cos(2*jnp.pi*(0.68+jnp.arange(0,T)/12)))*(result[:,2]+p2*result[:,5]+pA*result[:,8])/(result[:,0]+result[:,1]+result[:,2]+result[:,3]+result[:,4]+result[:,5]+result[:,6]+result[:,7]+result[:,8]+result[:,9])
H = h*d1*foi*result[:,1]+h*d2*s1*foi*result[:,4]

# Plot the results
plt.plot(H, label='Hospitalizations')
plt.ylim(0, 1500)
plt.xticks(jnp.arange(0, T+1, 12), jnp.arange(2005-30, 2005+1))
