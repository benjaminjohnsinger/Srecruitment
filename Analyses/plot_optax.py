...
if re.search(r'\d{6}',lockdown):
    with open("Data/Processed/results"+str(seed)[:6]+"/optax_"+pathogen+"FlexStepwise"+option1+option2+str(seed)+".pickle","rb") as f:
        opt = pickle.load(f)
else:
    try:
        with open("Data/Processed/results"+str(seed)[:6]+"/optax_"+pathogen+lockdown+option1+option2+str(seed)+".pickle","rb") as f:
            opt = pickle.load(f)
...
fig, ax = plt.subplots(1,2,figsize=(14.5,2.8))
ax[0].plot(neglogLs)
ax[0].set_title("Negative log-likelihood history")
ax[0].set_xlabel("Iteration")
ax[0].set_ylabel("Negative log-likelihood")
ax[1].plot(neglogLs[-200:])
ax[1].set_title("Negative log-likelihood history (last 200 iterations)")
