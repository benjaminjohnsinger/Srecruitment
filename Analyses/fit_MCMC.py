...
def SIS_likelihood(data, params, POINTS, STATE0, OBS_AGE, p_time_to_obs, age=True, incidence=True, start_t=date_to_t(pd.to_datetime('1970-01-01')), overdispersion=False, result=None):
    ...
    if not age:
        trajectory = jnp.sum(trajectory,axis=1)
    ...
    if incidence:
        if age:
            NAG = params["NAG"]
            N_S = params["N_S"]
            cases = jnp.round(data*jnp.array([jnp.sum(ry[range(i,(N_S*N_C+1)*NAG,NAG),len(p_time_to_obs):],axis=0) for i in range(NAG)]).T)
        else:
            cases = data*jnp.sum(ry,axis=0)
    else:
        cases = data.copy()
    ...
    expected_obs = jnp.sum([jnp.roll(trajectory,i,axis=0)*p_time_to_obs[i] for i in range(len(p_time_to_obs))],axis=0)
...
def mcmc(data, init_params, POINTS, STATE0, OBS_AGE, likelihood, p_time_to_obs, variables, initial_scalars, log_priors, proposal_cov, n_iter, age=False, incidence=False,n_messages=20):
    n_v = len(variables)
    NAG = init_params["NAG"]
    acceptance = jnp.zeros(n_iter)
    param_trajectory = jnp.zeros((n_iter+1,n_v))
    param_trajectory[0] = initial_scalars
    likelihoods = jnp.zeros(n_iter)
    ...
    for i in range(n_iter):
        if i % (n_iter//n_messages) == 0:
            print(f"Progress: {i/n_iter*100:.0f}%, acceptance rate: {jnp.mean(acceptance[i-(n_iter//n_messages):i])*100:.4f}%, time elapsed: {time.time()-start:.2f}s")
        proposal_params = current_params.copy()
        scalars = param_trajectory[i]
        scalars = real_to_p(p_to_real(scalars) + jnp.random.multivariate_normal(jnp.zeros(n_v),proposal_cov))
        ...
        if log_likelihood_diff > 0 or jnp.log(jnp.random.rand()) < log_likelihood_diff:
    ...
    return param_trajectory, jnp.mean(acceptance), likelihoods
...
def mcmc_adaptive(data, init_params, POINTS, STATE0, OBS_AGE, likelihood, p_time_to_obs, variables, initial_scalars, log_priors, step_var, n_iter, age=False, incidence=False,n_messages=20):
    n_v = len(variables)
    NAG = init_params["NAG"]
    acceptance = jnp.zeros((n_iter,n_v))
    param_trajectory = jnp.zeros((n_iter+1,n_v))
    param_trajectory[0] = initial_scalars
    likelihoods = jnp.zeros(n_iter)
    current_params = init_params.copy()
    ...
    for i in range(n_iter):
        if i % (n_iter//n_messages) == 0:
            # print(f"Progress: {i/n_iter*100:.0f}%, acceptance rate: {jnp.mean(acceptance[i-(n_iter//n_messages):i])*100:.4f}%, variance: {step_var}, time elapsed: {time.time()-start:.2f}s")
            # print each step_var value to 2 significant figures
            print(f"Progress: {i/n_iter*100:.0f}%, acceptance rate: {[f'{x:.4f}' for x in jnp.mean(acceptance[i-(n_iter//n_messages):i],axis=0)*100]}%, variance: {[f'{x:.2g}' for x in step_var]}, time elapsed: {time.time()-start:.2f}s")
        if i > 20:
            acceptance_rate = jnp.mean(acceptance[i-20:i],axis=0)
            update = (acceptance_rate - 0.23)/jnp.log(i+1)
            step_var = step_var*jnp.exp(update)
        random_variable_order = jnp.random.permutation(n_v)
        for i_v in random_variable_order:
            proposal_params = current_params.copy()
            scalars = param_trajectory[i]
            scalars[i_v] = real_to_p(p_to_real(scalars[i_v]) + jnp.random.normal(0,step_var[i_v]))
            scalar_dict = {variables[j]:scalars[j] for j in range(n_v)}
            proposal_params = scalars_to_params(scalar_dict,proposal_params)
            if age:
                proposal_OBS_AGE = age_detection(NAG,*scalars[-3:])
    ...
    return param_trajectory, jnp.mean(acceptance,axis=0), likelihoods