...
def age_detection(NAG,young_immunity,old_immunity,young_old,maternal_immunity=None,linear=True,min_obs=0.05,n_infant_groups=2):
    ...
    if maternal_immunity is None:
        x = jnp.arange(NAG)
        if not linear:
            OBS = jnp.minimum(1,2*young_old)*young_immunity**x+jnp.minimum(1,2*(1-young_old))*old_immunity**(NAG-1-x)
        elif linear:
            OBS = jnp.max((min_obs*jnp.ones(NAG),jnp.minimum(1,2*young_old)-young_immunity*x,jnp.minimum(1,2*(1-young_old))-old_immunity*(NAG-1-x)),axis=0)
    else:
        x = jnp.arange(NAG-n_infant_groups)
        OBS_noninfant = jnp.max((min_obs*jnp.ones(NAG-n_infant_groups),jnp.minimum(1,2*young_old)-young_immunity*x,jnp.minimum(1,2*(1-young_old))-old_immunity*(NAG-1-n_infant_groups-x)),axis=0)
        OBS_infant = jnp.max((min_obs*jnp.ones(n_infant_groups),jnp.min((jnp.ones(n_infant_groups),jnp.minimum(1,2*young_old)-2*(maternal_immunity-0.5)*jnp.arange(n_infant_groups,0,-1)),axis=0)),axis=0)
        OBS = jnp.concatenate((OBS_infant,OBS_noninfant))
    return OBS/jnp.max(OBS)

    # else:
    #     if maternal_immunity <= young_immunity:
    #         OBS_infant = jnp.max((min_obs*jnp.ones(n_infant_groups),jnp.minimum(1,2*young_old)-(young_immunity-maternal_immunity)*jnp.arange(n_infant_groups),axis=0))
    #         young_start = jnp.minimum(1,2*young_old)-n_infant_groups*(young_immunity-maternal_immunity)
    #     else:
    #         m_i = (maternal_immunity-young_immunity)/(1-young_immunity)
    #         OBS_infant = jnp.max((min_obs*jnp.ones(n_infant_groups),jnp.minimum(1,2*young_old)-mi*jnp.arange(n_infant_groups,0,-1),axis=0))
    #         young_start = jnp.minimum(1,2*young_old)
    #     x = jnp.arange(NAG-n_infant_groups)
    #     OBS_noninfant = jnp.max((min_obs*jnp.ones(NAG-n_infant_groups),young_start-young_immunity*x,jnp.minimum(1,2*(1-young_old))-old_immunity*(NAG-1-n_infant_groups-x)),axis=0)
...
def scalars_to_params(scalar_values_dict, params, NAG=7, N_S=3, N_C=2):
    for name in scalar_values_dict.keys():
        if name in ["NAG","N_S","BIRTH_RATE","S_VAX","ACOV","BCOV","T_VAX","IMPORT_RATE","BETA","SEASONALITY","OFFSET"]:
            ...
            params[name] = scalar_values_dict[name]*jnp.array([0]+[1]*(N_S-2)+[0])
        elif name == "REC_UP":
            params[name] = scalar_values_dict[name]*jnp.array([1]*(N_S-1)+[0])
        elif name == "REC_SAME":
            params[name] = scalar_values_dict[name]*jnp.array([0]*(N_S-1)+[1])
        elif name == "REC":
            params["REC_UP"] = scalar_values_dict[name]*jnp.array([1]*(N_S-1)+[0])
            params["REC_SAME"] = scalar_values_dict[name]*jnp.array([0]*(N_S-1)+[1])
        elif name == "P_OBS":
            params[name] = scalar_values_dict[name]*params[name]/jnp.max(params[name])
        elif name == "P_OBS_REL":
            params["P_OBS"] = jnp.max(params[name])*increment_to_vec(scalar_values_dict[name],N_S)
            ...
            i = int(name[5:])
            params["S_REL"][i] = jnp.prod(jnp.array([scalar_values_dict["S_REL"+str(j)] for j in range(1,i+1)]))
        elif re.search(r"D_REL\d+",name):
            i = int(name[5:])
            pobsrel = jnp.array([1]+[jnp.prod(jnp.array([scalar_values_dict["D_REL"+str(j)] for j in range(1,i+1)])) for i in range(1,N_S)])
            ...
            Ts = jnp.array([date_to_t('1970-01-01'),date_to_t('2020-03-19'),date_to_t('2020-03-19')+scalar_values_dict["DT1"]*365,date_to_t('2020-03-19')+(scalar_values_dict["DT1"]+scalar_values_dict["DT2"])*365,date_to_t('2020-03-19')+(scalar_values_dict["DT1"]+scalar_values_dict["DT2"]+scalar_values_dict["DT3"])*365])
            ...
            CONTACT = jnp.asarray(pd.read_csv('Data/Processed/contact_matrices/KP_contact_all_US_Census.csv', delimiter=',', dtype=jnp.float32, header=None).values)
            # @jit
    ...
    #     params["OBS_AGE"] = age_immunity(jnp.arange(NAG),scalar_values_dict["OBS_AGE_YOUNG"],scalar_values_dict["OBS_AGE_OLD"],scalar_values_dict["OBS_AGE_YOUNG_OLD"])
    return params
...
def params_to_scalars(param_dict,scalar_names):
    ...
    for name in scalar_names:
        if name in ["NAG","N_S","BIRTH_RATE","S_VAX","ACOV","BCOV","T_VAX","IMPORT_RATE","BETA","SEASONALITY","OFFSET"]:
            ...
            scalar_dict[name] = param_dict["P_OBS"][1]/jnp.max(param_dict["P_OBS"])
        elif name == "ACQUIRED_IMMUNITY":
...
def pathogen_parameters(pathogen, lockdown=None, CONTACT=None):
    ...
    if lockdown == 'Mobility':
        ...
        # @jit
        def contact(t,seasonality,offset):
            return CONTACT*(1+seasonality*jnp.cos(2*jnp.pi*((t-274)/365-offset)))
    elif lockdown == 'YoungEarly':
        # @jit
        def contact(t,seasonality,offset):
            cont = cm.google_prestige_work(t)*(1+seasonality*jnp.cos(2*jnp.pi*((t-274)/365-offset)))*CONTACT
            if t > 18702: # 2021-03-16 where majority of schoools returned to in-person according to burbio
                cont[0:4] = (1+seasonality*jnp.cos(2*jnp.pi*((t-274)/365-offset)))*CONTACT[0:4]
            return cont
    else:
    ...
    if pathogen == 'RSV':
        ...
        'contact': contact}
        p_time_to_obs = jnp.asarray(pd.read_csv("Data/Processed/RSV_incubation_admittance_distribution.csv",delimiter=',',dtype=jnp.float32, header=None).values)
        incidence = jnp.asarray(pd.read_csv("Data/Processed/KPSC_RSV_incidence_age_daily.csv",index_col=0))
        ...
        # Parameters for the ODE
        params = {'NAG': NAG, 'N_S': N_S, 'AGING_RATE': AGING_RATE, 'BIRTH_RATE': birth_rate, 'WANE': WANE, 'REC_UP': REC_UP, 'REC_SAME': REC_SAME, 'S_REL': S_REL, 'S_AGE': S_AGE, 'I_REL': I_REL, 'P_OBS': P_OBS, 'birth_vax': birth_vax, 'S_VAX': S_VAX, 'ACOV': flu_rate, 'BCOV': BCOV,
        'arrivals': arrivals, 'regional_positivity': regional_positivity, 'IMPORT_RATE': IMPORT_RATE, 'BETA': BETA, 'SEASONALITY': SEASONALITY, 'OFFSET': OFFSET,
        'contact': contact}
        p_time_to_obs = jnp.asarray(pd.read_csv("Data/Processed/Influenza_A_incubation_admittance_distribution.csv",delimiter=',',dtype=jnp.float32, header=None).values)
        incidence = jnp.asarray(pd.read_csv("Data/Processed/KPSC_Influenza_A_incidence_age_daily.csv",index_col=0))
        ...
        'arrivals': arrivals, 'regional_positivity': regional_positivity, 'IMPORT_RATE': IMPORT_RATE, 'BETA': BETA, 'SEASONALITY': SEASONALITY, 'OFFSET': OFFSET,
        'contact': contact}
        p_time_to_obs = jnp.asarray(pd.read_csv("Data/Processed/Influenza_B_incubation_admittance_distribution.csv",delimiter=',',dtype=jnp.float32, header=None).values)
        incidence = jnp.asarray(pd.read_csv("Data/Processed/KPSC_Influenza_B_incidence_age_daily.csv",index_col=0))
        ...
        'contact': contact}
        p_time_to_obs = jnp.asarray(pd.read_csv("Data/Processed/Influenza_A_incubation_admittance_distribution.csv",delimiter=',',dtype=jnp.float32, header=None).values)
        incidence = jnp.asarray(pd.read_csv("Data/Processed/KPSC_Parainfluenza3_incidence_age_daily.csv",index_col=0))
        ...
        'contact': contact}
        p_time_to_obs = jnp.asarray(pd.read_csv("Data/Processed/Influenza_A_incubation_admittance_distribution.csv",delimiter=',',dtype=jnp.float32, header=None).values)
        incidence = jnp.asarray(pd.read_csv("Data/Processed/KPSC_Adenovirus_incidence_age_daily.csv",index_col=0))
        ...
        'contact': contact}
        p_time_to_obs = jnp.asarray(pd.read_csv("Data/Processed/Influenza_A_incubation_admittance_distribution.csv",delimiter=',',dtype=jnp.float32, header=None).values)
        incidence = jnp.asarray(pd.read_csv("Data/Processed/KPSC_Metapneumovirus_incidence_age_daily.csv",index_col=0))
    return params, p_time_to_obs, incidence


####### Generating interesting quantities from ODE results #######

def observations(result, POINTS, NAG, N_S, BETA, contact, SEASONALITY, OFFSET, S_REL, I_REL, P_OBS, OBS_AGE,incidence=False,cap=False,N_C=2,time_conversion=30.44):
    ...
    obs = jnp.zeros((len(POINTS),NAG))
    pop_size = jnp.sum(result,axis=0)
    for i_t,t in enumerate(POINTS):
        foi = BETA*jnp.dot(contact(t,SEASONALITY,OFFSET),jnp.sum((jnp.array([result[(N_C*j+2)*NAG:(N_C*j+3)*NAG,i_t] for j in range(N_S)])*I_REL),axis=0))/pop_size[i_t]
        for i in range(N_S):
            if cap:
                class_foi = jnp.minimum(1,S_REL[i]*foi)
            else:
    ...
    if incidence:
        obs = jnp.sum(obs,axis=1)/pop_size
...
def infections_by_age(result,params,N_C=2):
    ...
    infs = jnp.zeros((len(result.t),NAG))
    for i_t,t in enumerate(result.t):
        infs[i_t,:] = jnp.sum(jnp.array([BETA*result.y[(N_C*i+2)*NAG:(N_C*i+3)*NAG,i_t]*I_REL[i]*jnp.dot(contact(t,SEASONALITY,OFFSET),jnp.sum([S_REL[j]*result.y[(N_C*j+1)*NAG:(N_C*j+2)*NAG,i_t] for j in range(N_S)],axis=0)) for i in range(N_S)]),axis=0)
    return(infs)
...
def age_of_first_infection(result,MEDIAN_AGE,NAG=7,sd=False):
    ...
    """
    ages = jnp.zeros(len(result.t))
    ages_sd = jnp.zeros(len(result.t))
    for i_t in range(len(result.t)):
        age_distribution = MEDIAN_AGE*result.y[2*NAG:3*NAG,i_t]/jnp.sum(result.y[2*NAG:3*NAG,i_t])
        ages[i_t] = jnp.mean(age_distribution)
        ages_sd[i_t] = jnp.std(age_distribution)
    if sd:
        return(ages,ages_sd)
    return(ages)
...
def susceptibility(result,params,N_C=2):
    ...
    sus = jnp.zeros((len(result.t),NAG))
    for i_t,t in enumerate(result.t):
        for i in range(N_S):
            sus = sus.at[i_t,:].add(S_REL[i]*S_AGE*result.y[(N_C*i+1)*NAG:(N_C*i+2)*NAG,i_t])
    return(sus)