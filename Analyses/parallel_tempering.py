...
def run_chain(temperature, q_in, q_out, n_iterations, initial_state, proposal_widths, compute_energy, swap_interval=1, save_interval=1, samples_queue=None):
    ...
    acceptance = jnp.zeros((n_iterations, n_params))
    ...
    for i in range(n_iterations):
        ...
        if i > 20:
            ...
            acceptance_rates = jnp.mean(acceptance[i-20:i], axis=0)
            # Update proposal widths (targeting 23% acceptance rate)
            update = (acceptance_rates - 0.23) / jnp.log(i+1)
            proposal_widths = proposal_widths * jnp.exp(-update)
        ...
        param_order = jnp.random.permutation(n_params)
        ...
        for param_idx in param_order:
            ...
            proposal[param_idx] += jnp.random.normal(0, proposal_widths[param_idx])
            ...
            if delta_e < 0 or jnp.random.random() < jnp.exp(-delta_e / temperature):
    ...
    return jnp.mean(acceptance, axis=0)
...
def coordinator(n_chains, temperatures, q_chains_out, q_chains_in, n_swaps):
    ...
    swap_attempts = jnp.zeros((n_chains-1))
    swap_accepts = jnp.zeros((n_chains-1))
    ...
    for swap_idx in range(n_swaps):
        ...
        for i in range(n_chains - 1):
            ...
            if delta < 0 or jnp.random.random() < jnp.exp(-delta):
                # Accept swap
...
def neg_log_likelihood(state, variables, params, data, POINTS, STATE0):
    if "OVERDISPERSION" in variables:
        overdispersion = jnp.exp(state[-1]-5)
        vbls = variables[:-1]
...
ordered_variables = jnp.array([var for var in ["WANE","SEASONALITY","OFFSET","BETA","IMPORT_RATE","EXTRA_IMMUNITY","FIRST_IMMUNITY","FIRST_DIS_INF_FACTOR","S_REL1","S_REL2","D_REL1","D_REL2","P_OBS","DT1","DT2","DT3","F1","F2","F3","F4","OVERDISPERSION","AGE_OBS_YOUNG","AGE_OBS_OLD","AGE_OBS_YOUNG_OLD"]
if var in variables])
...
if __name__ == "__main__":
    ...
    temperatures = jnp.exp(jnp.linspace(0, 3, n_chains))  # from 1.0 to ~20.1
    ...
    proposal_widths = jnp.ones(len(initial_state)) * 0.1
    ...
    proposal_history = jnp.array(proposal_history)
    
    # Calculate parameter-wise transition rates
    transitions = jnp.sum(samples[1:] != samples[:-1], axis=0)
    ...
    print(f"Mean: {real_to_p(jnp.mean(samples, axis=0))}")
    print(f"Median: {real_to_p(jnp.median(samples, axis=0))}")
    print(f"Std: {jnp.std(samples, axis=0)}")
    print(f"IQR: {jnp.percentile(samples, 75, axis=0) - jnp.percentile(samples, 25, axis=0)}")
