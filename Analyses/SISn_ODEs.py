...
def single_pathogen_deltas(t,state,params):
    ...
    NAG, N_S, AGING_RATE, BIRTH_RATE, WANE, REC, S_REL, S_AGE, I_REL, P_OBS, birth_vax, all_vax, S_VAX, ACOV, BCOV, T_VAX, IMPORT, BETA, contact = params
    ...
    for i in range(N_S):
        ...
        delta[(2*i)*NAG:(2*i+1)*NAG] = birth_vax(t,i,S_VAX,BCOV,T_VAX)*BIRTH_RATE*pop_size*np.concatenate((np.ones(1),np.zeros(NAG-1)))\
            ...
            + (all_vax(t,i,ACOV,S_VAX,T_VAX,NAG,N_S)*state).reshape((2*N_S+1,NAG)).sum(axis=0)
