...
def cluster_sims(results,obses,T_LOCKDOWN,n_clusters,pre=True,width=5,scaled=False,metric='euclidean'):
    ...
    for o_key,obs in obses.items():
        ...
        if scaled:
            obs_temp = obs_temp/jnp.max(obs_temp)
    ...
    model.fit(jnp.array([obs_cut_and_scaled[key] for key in obs_cut_and_scaled.keys()]))
    return model
...
def cluster_first_infect(results,params,model,T_LOCKDOWN,MEDIAN_AGE):
    ...
    for i in range(n_clusters):
        first_infect = jnp.zeros(sum(result_cluster_labels==i))
        aidx = 0
        for j,key in enumerate(results.keys()):
            if result_cluster_labels[j]==i:
                first_infect[aidx] = jnp.mean(age_of_first_infection(results[key],MEDIAN_AGE)[jnp.argmax(results[key].t>=T_LOCKDOWN-5*12):jnp.argmax(results[key].t>T_LOCKDOWN)])
                aidx += 1
        cluster_mean_first_infect[i] = jnp.mean(first_infect)/12
