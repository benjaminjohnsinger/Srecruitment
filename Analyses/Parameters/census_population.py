...
AGE_POP = np.asarray(pd.read_csv('Data/Processed/US_Census_population_by_age.csv', header=None).values) # US in 2022
AGE_GROUPS = [
    np.arange(3),
    np.arange(3,12),
    np.arange(12,5*12),
    np.arange(5*12,8*12),
    np.arange(8*12,40*12),
...
MEDIAN_AGE = [np.median(group) for group in AGE_GROUPS]
MEDIAN_AGE_split = [np.median(group) for group in AGE_GROUPS_split]
# AGE_GROUP_NAMES = [
#     'Newborns',
#     'Infants',
#     'Young children',
...
AGE_PROPORTION_split = np.array([np.sum(AGE_POP[group]) for group in AGE_GROUPS_split])/np.sum(AGE_POP)
# age_array = [x for xs in [[i]*AGE_POP[i] for i in group] for x in xs]
# print(MEDIAN_AGE)
# print([np.median(np.array([x for xs in [[i]*int(AGE_POP[i]) for i in group] for x in xs])) for group in AGE_GROUPS])
...
AGING_RATE = 1/np.array([3/12*365,9/12*365,4*365,3*365,32*365,25*365,12.43*365])
AGING_RATE_split = 1/np.array([3/12*365,9/12*365,4*365,3*365,10*365,22*365,25*365,12.43*365])
