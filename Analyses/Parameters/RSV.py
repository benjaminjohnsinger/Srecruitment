...
# WANE = jnp.array([0,1.195e-01,0])/365
# Recovery rates for each susceptibility class
REC_UP = jnp.array([1/4.9,1/4.1,0.0]) # Recovery to higher susceptibility class - Okiro 2010
REC_SAME = jnp.array([0.0,0.0,1/4.1]) # Recovery to same susceptibility class - Okiro 2010
# Relative susceptability and infectiousness, for each susceptibility class
# S_REL = jnp.array([1,0.559,0.333]) # Glezen 1986
S_REL = jnp.array([1,0.25,0.025]) # Nokes 2008
I_REL = jnp.array([[1],[1],[1]])
# Probability of detection of cases for each susceptibility class
P_OBS_REL = jnp.array([1,0.46,0.31]) # Henderson 1979 - assuming that hospitalization risk is proportional to LRTI risk (an assumption borrowed from Ginny Pitzer), and 2nd and 3rd infections probably occur in older children
# P_OBS_MAX = 3.396e-02
P_OBS_MAX = 0.03
...
def regional_positivity(t, PP_NP=PP_NP, PP_IDX=PP_IDX):
    ...
    return jnp.where((t < PP_IDX[0]) | (t > PP_IDX[-1]),
                    PP_NP[jnp.argmax(PP_IDX >= time_2010)],
                    PP_NP[jnp.argmax(PP_IDX >= t)])