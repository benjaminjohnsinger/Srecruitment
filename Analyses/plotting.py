...
def lockdown_incidence_plot(ax,state0,params,OBS_AGE,period,points,T_LOCKDOWN,LOCKDOWN_DURATION,result=None,label='Observed cases',color='#648FFF',linewidth=1,alpha=1,by_age=False,AGE_GROUP_NAMES=None,relative=False,deltas=deltas_SIS,obs=None,times=None,start_t=date_to_t('2015-10-01'),end_t=date_to_t('2023-09-30'),factor=1,p_time_to_obs=[1]):
    ...
    start_index = jnp.argmin(times<=start_t)
    end_index = jnp.argmin(times<=end_t)
    if end_index <= start_index:
        end_index = len(times)
    if by_age:
        pop_size_by_age = jnp.array([jnp.sum(ry[range(i_age,(N_C*N_S+1)*NAG,NAG),:],axis=0) for i_age in range(NAG)]).T
        if obs is None:
        ...
        obs = jnp.sum(jnp.array([jnp.roll(obs,i,axis=0)*p_time_to_obs[i] for i in range(len(p_time_to_obs))]),axis=0)
        ...
        mx = 1.1*jnp.max(jnp.max(obs/pop_size_by_age,axis=1)[start_index:end_index])
    else:
        if obs is None:
            obs = factor*observations(result,params,OBS_AGE,incidence=True)
        else:
            obs = factor*obs
        obs = jnp.sum(jnp.array([jnp.roll(obs,i)*p_time_to_obs[i] for i in range(len(p_time_to_obs))]),axis=0)
        if relative:
            pre_mx = jnp.max(obs[start_index:jnp.argmin(times<=T_LOCKDOWN)])
...
def lockdown_incidence_format(ax,T_LOCKDOWN,LOCKDOWN_DURATION,mx,year_window=5,year_skip=1,title='Incidence of disease'):
    ...
    # ax.set_xticks(jnp.arange(T_LOCKDOWN-year_window*365,T_LOCKDOWN+LOCKDOWN_DURATION+year_window*365+365,365*year_skip),[str(int(x)-year_window-1) for x in jnp.arange(0,year_window*2+2,year_skip)])
...
def lockdown_susceptibility_plot(ax,state0,params,period,points,T_LOCKDOWN,result=None,label='Susceptible_population',color='#648FFF',relative=True,proportion=False,by_age=False,AGE_GROUP_NAMES=None,style='-',delta=deltas_SIS):
    ...
    if by_age:
        if proportion:
            pop_by_age = jnp.array([jnp.sum(result.y[range(i_age,(N_C*N_S+1)*NAG,NAG),:],axis=0) for i_age in range(NAG)]).T
            sus = sus/pop_by_age
        for i in range(NAG):
            ax.plot(dates,sus[:,i], label=AGE_GROUP_NAMES[i], color=hsv_colors[i],linestyle=style)
    else:
        total_sus = jnp.sum(sus,axis=1)
        if relative:
            pre_mx_sus = jnp.mean(total_sus[jnp.argmax(result.t>T_LOCKDOWN-5*365):jnp.argmax(result.t>T_LOCKDOWN)])
...
def lockdown_susceptibility_format(ax,T_LOCKDOWN,LOCKDOWN_DURATION,ymin=0.875,ymax=1.1,year_window=5):
    # ax.set_xlim(T_LOCKDOWN-year_window*365,T_LOCKDOWN+LOCKDOWN_DURATION+year_window*365)
    # ax.set_xticks(jnp.arange(T_LOCKDOWN-year_window*365,T_LOCKDOWN+LOCKDOWN_DURATION+year_window*365,365),[str(int(x)-year_window) for x in jnp.arange(0,2*year_window+1,1)])
    ax.set_xlabel('Time (years)')
...
def age_infect_plot(ax,state0,params,AGE_GROUP_NAMES,period,points,T_LOCKDOWN,LOCKDOWN_DURATION,result=None,delts=deltas_SIS):
    ...
    rel_infs = infs/jnp.sum(infs,axis=1)[:,jnp.newaxis]
    for i in range(NAG):
    ...
    ax.set_xticks(jnp.arange(T_LOCKDOWN-5*365,T_LOCKDOWN+LOCKDOWN_DURATION+5*365,365),[str(int(x)-5) for x in jnp.arange(0,11,1)])
    ax.set_xlabel('Time (years)')
    ax.set_ylim(0,1)
    # ax.legend()
...
def param_line_plot(ax,results,params,T_LOCKDOWN,LOCKDOWN_DURATION,OBS_AGE=None,
    ...
    values = jnp.zeros((N,len(y_values)))
    parameter_values = jnp.zeros(N)
    ...
    for p_n,result in results.items():
        ...
        for p in p_n:
            for pname in grid_params:
                if grid_mode == "scale":
                    ...
                    vec = jnp.array([(1-j*(p/(N*(vec_len-1))))**factor for j in range(vec_len)])
            ...
            if grid_mode == "fade_vec":
                ...
                parameter_values[p] = jnp.mean(params_n[grid_params[0]]) - jnp.mean(params_n[grid_params[1]])
            elif label_mode=="mean":
                parameter_values[p] = jnp.mean([jnp.mean(params_n[pname]) for pname in grid_params])
            elif label_mode=="nz_mean":
                parameter_values[p] = jnp.mean([jnp.mean(params_n[pname])*(len(params_n[pname])/jnp.sum(params_n[pname]!=0)) for pname in grid_params])
        ...
        if ("peak incidence" in y_values) or ("time to rebound" in y_values) or ("rebound peak incidence"in y_values) or ("periodicity" in y_values):
            ...
            if "peak incidence" in y_values:
                values[p_n,[idx for idx in range(len(y_values)) if y_values[idx]=="peak incidence"]] = jnp.max(obs[(result.t>T_LOCKDOWN-12*365) & (result.t<T_LOCKDOWN)])
            if "time to rebound" in y_values:
                post_peak_arg = jnp.argmax(obs[result.t>=(T_LOCKDOWN+LOCKDOWN_DURATION)] > jnp.max(obs[(result.t>T_LOCKDOWN-12*365) & (result.t<T_LOCKDOWN)])/2)
                val = result.t[jnp.argmax(result.t>=(T_LOCKDOWN+LOCKDOWN_DURATION))+post_peak_arg]-(T_LOCKDOWN+LOCKDOWN_DURATION)
            ...
            if "rebound peak incidence" in y_values:
                values[p_n,[idx for idx in range(len(y_values)) if y_values[idx]=="rebound peak incidence"]] = jnp.max(obs[result.t>=(T_LOCKDOWN+LOCKDOWN_DURATION)])
        ...
        if "child infections" in y_values or "under-five infections" in y_values:
            ...
            if "child infections" in y_values:
                values[p_n,[idx for idx in range(len(y_values)) if y_values[idx]=="child infections"]] = (jnp.sum(infs[:,0:4],axis=1)/jnp.sum(infs,axis=1))[jnp.argmax(result.t>=T_LOCKDOWN)]
            if "under-five infections" in y_values:
                values[p_n,[idx for idx in range(len(y_values)) if y_values[idx]=="under-five infections"]] = (jnp.sum(infs[:,0:3],axis=1)/jnp.sum(infs,axis=1))[jnp.argmax(result.t>=T_LOCKDOWN)]
        if "pre-lockdown susceptibility" in y_values or "post-lockdown susceptibility" in y_values:
            ...
            if "pre-lockdown susceptibility" in y_values:
                values[p_n,[idx for idx in range(len(y_values)) if y_values[idx]=="pre-lockdown susceptibility"]] = total_sus[jnp.argmax(result.t<=T_LOCKDOWN)]
            if "post-lockdown susceptibility" in y_values:
                values[p_n,[idx for idx in range(len(y_values)) if y_values[idx]=="post-lockdown susceptibility"]] = total_sus[jnp.argmax(result.t>=T_LOCKDOWN+LOCKDOWN_DURATION)]
...
def grid_plot(ax,results,params,T_LOCKDOWN,LOCKDOWN_DURATION,OBS_AGE=None,
    ...
    N = max([max(p) for p in results.keys()])+1
    z_values = jnp.zeros(jnp.repeat(N,N_params))
    parameter_values = jnp.zeros((N,N_params))
        ...
        for p_n,result in results.items():
            ...
            for i,p in enumerate(p_n):
                ...
                if label_mode[i]=="diff_mean":
                    parameter_values[p,i] = jnp.mean(params_n[grid_params[i][0]]) - jnp.mean(params_n[grid_params[i][1]])
                elif label_mode[i]=="mean":
                    parameter_values[p,i] = jnp.mean([jnp.mean(params_n[pname]) for pname in grid_params[i]])
                elif label_mode[i]=="nz_mean":
                    parameter_values[p,i] = jnp.mean([jnp.mean(params_n[pname])*(len(params_n[pname])/jnp.sum(params_n[pname]!=0)) for pname in grid_params[i]])
            # Calculate observations
            if (z_value == "peak incidence") or (z_value == "min incidence") or (z_value == "oscillation size") or (z_value == "time to rebound") or (z_value == "rebound peak incidence") or (z_value == "periodicity"):
                ...
                if z_value == "peak incidence":
                    z_values[p_n] = jnp.max(obs[(result.t>T_LOCKDOWN-12*365) & (result.t<T_LOCKDOWN)])
                if z_value == "min incidence":
                    z_values[p_n] = jnp.min(obs[(result.t>T_LOCKDOWN-12*365) & (result.t<T_LOCKDOWN)])
                if z_value == "oscillation size":
                    pre_obs = obs[(result.t>T_LOCKDOWN-12*365) & (result.t<T_LOCKDOWN)]
                    z_values[p_n] = (jnp.max(pre_obs) - jnp.min(pre_obs))/jnp.mean(pre_obs)
                if z_value == "time to rebound":
                    post_peak_arg = jnp.argmax(obs[result.t>=(T_LOCKDOWN+LOCKDOWN_DURATION)] > jnp.max(obs[(result.t>T_LOCKDOWN-12*365) & (result.t<T_LOCKDOWN)])/2)
                    z_values[p_n] = (result.t[jnp.argmax(result.t>=(T_LOCKDOWN+LOCKDOWN_DURATION))+post_peak_arg]-(T_LOCKDOWN+LOCKDOWN_DURATION))/365
                if z_value == "rebound peak incidence":
                    z_values[p_n] = jnp.max(obs[result.t>=(T_LOCKDOWN+LOCKDOWN_DURATION)])
            ...
            if (z_value == "child infections") or (z_value == "under-five infections"):
                ...
                if z_value == "child infections":
                    z_values[p_n] = (jnp.sum(infs[:,0:4],axis=1)/jnp.sum(infs,axis=1))[jnp.argmax(result.t>=T_LOCKDOWN)]
                if z_value == "under-five infections":
                    z_values[p_n] = (jnp.sum(infs[:,0:3],axis=1)/jnp.sum(infs,axis=1))[jnp.argmax(result.t>=T_LOCKDOWN)]
            if (z_value == "pre-lockdown susceptibility") or (z_value == "post-lockdown susceptibility"):
                sus = susceptibility(result,params_n)
                total_sus = jnp.sum(sus,axis=1)
                if z_value == "pre-lockdown susceptibility":
                    z_values[p_n] = total_sus[jnp.argmax(result.t>=T_LOCKDOWN)]
                if z_value == "post-lockdown susceptibility":
                    z_values[p_n] = total_sus[jnp.argmax(result.t>=T_LOCKDOWN+LOCKDOWN_DURATION)]
    if save:
    ...
    if N_params == 2:
        im = ax.imshow(jnp.flipud(z_values))
        ...
        for i in range(N_params):
            for j in range(i+1,N_params):
                ...
                if fix:
                    ...
                    z_values_plot = jnp.mean(z_values,axis=tuple([k for k in range(N_params) if k not in [i,j]]))
                if (vmax is not None) and (vmin is not None):
                    im = axis.imshow(jnp.flipud(z_values_plot),vmin=vmin,vmax=vmax)
                else:
                    im = axis.imshow(jnp.flipud(z_values_plot))
...
def cluster_plot(axes,results,obses,n_clusters,labels,cluster_centers,relative=False,color=False,line=True,clusters=None, color_values_all=None,
    ...
    times = jnp.array(date_to_t(pd.date_range(start=pd.to_datetime(t_lockdown)-pd.Timedelta(5*365,unit='D'),end=pd.to_datetime(t_lockdown),freq='MS')))[:-1]
    param_values_all = jnp.zeros((len(results.keys()),len(parameters)))
    ...
    for i,cluster in enumerate(clusters):
        idx = jnp.where(labels==cluster)[0]
        values = jnp.zeros(len(idx))
        mx = 0
        param_values = param_values_all[idx,:]
        if color:
            if color_values_all is None:
                color_values = 0.95*(param_values-jnp.min(param_values_all,axis=0))/(jnp.max(param_values_all,axis=0)-jnp.min(param_values_all,axis=0))
            else:
        ...
        for n_j,j in enumerate(idx):
            ...
            if y_value == "rebound peak incidence":
                values[n_j] = jnp.max(obs[result.t>=(T_LOCKDOWN+LOCKDOWN_DURATION)])
            elif y_value == "time to rebound":
                post_peak_arg = jnp.argmax(obs[result.t>=(T_LOCKDOWN+LOCKDOWN_DURATION)] > jnp.max(obs[(result.t>T_LOCKDOWN-5*365) & (result.t<T_LOCKDOWN)])/2)
                val = result.t[jnp.argmax(result.t>=(T_LOCKDOWN+LOCKDOWN_DURATION))+post_peak_arg]-(T_LOCKDOWN+LOCKDOWN_DURATION)
                values[n_j] = min(val/365,5)
            elif y_value == "child infections":
                infs = infections_by_age(result,params)
                values[n_j] = (jnp.sum(infs[:,0:4],axis=1)/jnp.sum(infs,axis=1))[jnp.argmax(result.t>=T_LOCKDOWN)]
            if color:
                if jnp.random.rand() < 500/len(idx):
        ...
        for n_p,param in enumerate(parameters):
            if color and not line:
                jitter_param = jnp.random.normal(-1,1,len(param_values[:,n_p]))*base_values[n_p]/(2*N)
                jitter_values = jnp.random.normal(-1,1,len(param_values[:,n_p]))*(1/24)
                axes[i,n_p+1].scatter(param_values[:,n_p]+jitter_param,values+jitter_values,c=color_values,alpha=1,s=15/jnp.sqrt(len(idx)),linewidths=0)
            elif not line:
                axes[i,n_p+1].scatter(param_values[:,n_p],values,color="black",alpha=0.3,s=10)
            else:
                pf = param_values[:,n_p]
                Qs = jnp.zeros((len(pf),3))
                for pidx in range(len(pf)):
                    Qs[pidx,:] = jnp.percentile(values[pf==pf[pidx]],[25,50,75])
                sort_args = jnp.argsort(pf)
...
def kpsc_positive_test_plot(ax,hospitalizations=True,pathogen="RSV",AGE_GROUPS=None,AGE_GROUP_NAMES=None,incidence=False, color=hsv_colors, title=None, legend=True, aggregation=None, save_data=False):
    ...
    if incidence:
        ...
        # age_by_year.drop(columns=["Newborns"],ijnplace=True)
        if AGE_GROUPS is not None:
            age_by_year.columns = AGE_GROUP_NAMES
        age_by_year["Year"] = jnp.arange(2015,2023)
    ...
    if AGE_GROUPS is not None:
        ...
        cases["Count"] = cases["Count"]/jnp.sum(age_by_year.loc[cases.index.year].values,axis=1)
...
def season_sizes(cases):
    ...
    season_cumulative = jnp.zeros((len(seasons)-1,7))
    season_relative = jnp.zeros((len(seasons)-1,7))
...
def season_plot(ax,pathogen,incidence=False,relative=False):
    ...
    print(pathogen,jnp.sum(season_cumulative,axis=1))
    ...
    if incidence or relative:
        for i in range(season_cumulative.shape[0]):
            if jnp.sum(season_cumulative.iloc[i,:])<=37:
