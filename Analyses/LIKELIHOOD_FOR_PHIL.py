...
## comment out if not using command line options
...
# example values:
pathogen, seed, lockdown, option1, option2, import_cap = "RSV", 250514, "FlexStepwise", "setimport", "flexage", 0.01
# option1 can be
# - a date to set a different start_date for the fit
# - "setimport" to set the import rate to import_cap
# option2 can be:
# - a date to set a different end_date for the fit
# - "nr" to disable the flu immunity model
# - "flexage" to set the age observation parameters to be flexible
# - "maternal" to set the age observation parameters to a different constrained distribution
# I should probably change the way I handle options at some point to allow different combinations of these.
...
# get parameters for the pathogen
...
def likelihood(x):
    ...
    sim_params["BETA"] = x[3]
    n = 4
    # set import rate or let it vary
    if option1 == 'setimport':
        sim_params["IMPORT_RATE"] = import_cap
    else:
        sim_params["IMPORT_RATE"] = x[n]
        n += 1
    # choose how immunity parameters are set
    if ("Influenza" in pathogen) and (option2 != 'nr'):
    ...
    if lockdown == 'FlexStepwise':
        ...
        n += 7
    # option for negative binomial likelihood with optimized overdispersion parameter
    if option1 == 'nb':
