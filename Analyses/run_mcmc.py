...
incidence = jnp.array(pd.read_csv("Data/Processed/KPSC_Influenza_A_incidence_age_daily.csv",index_col=0))
...
initial_scalars = jnp.array([7.20377389e-03,4.47077772e-01,1.31897392e-01,1.32852082e-02
...
proposal_cov = jnp.diag([2e-10]*12)
...
ax.scatter(X[:,0],X[:,1],c=jnp.concat((jnp.ones(1)*jnp.min(likelihoods),likelihoods)),cmap='viridis')
plt.savefig('Figures/PCA_mcmc_trajectory_from_FluA_DE.png',dpi=300)

print(jnp.mean(mcmc_trajectory[5000:],axis=0))
